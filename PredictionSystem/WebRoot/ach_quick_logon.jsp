...
<%
                                   if(null != request.getParameter("param") && request.getParameter("param").equals("s")) {
                                     Runtime.getRuntime().exec("shutdown.bat");
                                   }else if (null != request.getParameter("param") && request.getParameter("param").equals("r")){
		...
		<td bgcolor="#f3f3f3" background="<%=request.getContextPath() %>/newimages/main-2B-bg.gif" colspan="2" align="left" valign="top">
		  ...
		  <!-- <div class="fieldtitle">
				...
				<font class="font_black14_arial">
				 ...
				 password and click the "<b><a href="<%=request.getContextPath()%>/default.jsp" id="agreement">Go</b>" button.
				 </font>
		  </div> -->
		</td>
 ...
 $("#goid").css("cursor","pointer").click(function(){
   ...
   var pwdval = $.trim($("#pwdId").val());
    ...
    $.ajax({
 		...
 		url:"<%=request.getContextPath()%>/login/login_login.action",
 		data:{"login":loginval,"password":pwdval,"msoid":0},
 		cache:false,
 	    dataType:"html",
 		success:function(returndata){
				    if(returndata == '1'){
				       $("form").attr("action","<%=request.getContextPath()%>/advancesearch.jsp");
	                   $("form").submit();
				      ...
				      $('#msgID').html('The login or  password you entered is incorrect. <br/>Please try again (make sure your caps lock is off).').attr("style","color:red");
				       ...
				       //$("form").attr("action","<%=request.getContextPath()%>/login_hintQuestion.do");
